...
AI 时代把一个难题变得更难了。你注册了 ChatGPT、Claude、Midjourney——每个 AI 工具都想要一个新账号，每个账号都想要一个新密码。再算上老的：邮箱、银行、淘宝、GitHub……

大多数人真正能记住的强密码，只有那么几个。于是每个人都妥协——而每种妥协都有代价：

- **所有账号一个密码** —— 一个网站泄露，全线沦陷，像多米诺骨牌
- **写在纸上、记在备忘录里** —— 纸会丢，备忘录会被翻到
- **让浏览器记住** —— 换浏览器、换电脑就全没了；Chrome 还会同步到谷歌云端
- **把密码交给 AI** ——"帮我整理一下我的密码"，正在变成一种很危险的习惯：你输入 AI 工具的每一个字，都会被读取、存储、用于训练
- **付费云密码管理器** —— 每年几十美元，密码库依然存在别人的服务器上（2022 年 LastPass 泄露了 2500 万用户的密码库）

再看看你的想法。AI 时代产出的想法比以往任何时候都多——prompt、草稿、半成品念头。三秒不记下来，就再也不会回来了。而打开云笔记？又要登录、又要同步，你的文字还要被服务器扫描。
...
而最要紧的是：**这一切都不会离开你的电脑。** 全部用 AES-256-GCM 加密。在数据比以往任何时候都值钱的时代，它就该待在它诞生的地方——你手里。你不需要信任我们，你不需要信任任何人。

**而且，它是开源的。** 整个应用只有几千行纯 JavaScript、依赖极少——小到任何人（或任何 AI 工具）都能通读全库、审计每一处加密调用、在此基础上继续开发。不喜欢哪里？自己改。想要我们还没做的功能？Fork 一份——AI 时代，不必等我们。
...
- 🧩 **开源，随你改造** —— 几千行无框架代码、依赖极少。可审计、可 Fork，也可以直接让 AI 工具帮你加功能。
